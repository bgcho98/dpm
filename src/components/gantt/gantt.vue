      ...
      .marker {
        ...
        &.parent {
          ...
          &:after {
            ...
            border-color: transparent transparent darken($markerColor, 40%)
              transparent;
            border-style: solid;
    ...
    <div class="gantt-chart" ref="gantt">
      ...
      <div class="gantt-header">
        ...
        <div class="table-section" :style="{ width: widths.table + 'px' }">
          <div
            class="table-cell"
            :class="{ sortable : !!field.sort }"
            :style="{ width: field.width + 'px' }"
            v-for="(field, slug) in localFields"
            :key="slug"
            @click="sortBy = slug"
          >{{ field.label }}</div>
        </div>
        ...
        <div class="graph-section" :style="{ width: widths.graph + 'px' }">
          <div
            class="table-cell"
            :class="{ 'hidden-date' : !date.shown }"
            :style="{ width: widths.cell + 'px' }"
      ...
      <div class="gantt-body">
        ...
        <div
          class="gantt-row"
          v-for="(item, key) in localSortedItems"
          :class="{ parent: item.isParent, child: item.isChild }"
          v-if="!item.isChild || (item.isChild && openItems.includes(item.parentId))"
          :key="key"
        >
          <!-- Table section: start -->
          <div class="table-section" :key="item.id" :style="{ width: widths.table + 'px'}">
            <template v-if="localFields.toggle">
              <div
                v-if="item.isParent"
                class="toggle"
                :style="{ width: localFields.toggle.width + 'px' }"
                @click="openItems.includes(item.id) ? openItems.splice(openItems.indexOf(item.id), 1) : openItems.push(item.id)"
              >
                <span
                  :class=" { open: openItems.includes(item.id), close: !openItems.includes(item.id) }"
                ></span>
              </div>
            ...
            <div class="table-group">
              <template
                v-if="localFields.hasOwnProperty(field)"
                v-for="(field, slug) in Object.keys(item)"
              >
                <gantt-user
                  v-if="localFields[field].component === 'gantt-user'"
                  class="table-cell"
                  v-model="item[field]"
                  :style="{ 'padding-left': (field === longest_cell.slug && item.isChild) ? indent + 'px' : ''  }"
                  :key="slug"
                  :width="localFields[field].width"
                  :user="user"
                  :editable="!item.isParent && localFields[field].editable"
                ></gantt-user>
                <gantt-text
                  v-if="localFields[field].component === 'gantt-text'"
                  class="table-cell"
                  v-model="item[field]"
                  :style="{ 'padding-left': (field === longest_cell.slug && item.isChild) ? indent + 'px' : ''  }"
                  :key="slug"
                ...
                <gantt-date
                  ...
                  :key="slug"
                  :width="localFields[field].width"
                  :minDate="item[localFields[field].minDate]"
                  @update="cellUpdated(localFields[field].callback, item, field)"
                  :editable="!item.isParent && localFields[field].editable"
                ></gantt-date>
                <gantt-number
                  v-if="localFields[field].component === 'gantt-number'"
                  class="table-cell"
                  v-model="item[field]"
                  :style="{ 'padding-left': (field === longest_cell.slug && item.isChild) ? indent + 'px' : ''  }"
                  :key="slug"
                  :width="localFields[field].width"
                  :min="localFields[field].min"
                  :max="localFields[field].max"
                  :suffix="localFields[field].suffix"
                  @update="cellUpdated(localFields[field].callback, item, field)"
                  :editable="!item.isParent && localFields[field].editable"
                ></gantt-number>
              </template>
          ...
          <div class="graph-section" :style="{ width: widths.graph + 'px' }">
            <gantt-graph
              :item="item"
              :dates="dates"
              :cell_width="widths.cell"
              :user="user"
              :open="!item.isParent || openItems.includes(item.id)"
              @date-updated="dateUpdated"
              @open="openItems.push(item.id)"
            ></gantt-graph>
          </div>
          <!-- Graph section:end -->
        </div>
  ...
  mounted() {
    ...
    Object.keys(this.localFields).forEach(field_key => {
      ...
      if (field_key === "start_date")
        this.localFields[field_key].callback = this.startdateUpdated;
      if (field_key === "end_date")
        this.localFields[field_key].callback = this.enddateUpdated;
      if (field_key === "end_date")
        this.localFields[field_key].minDate = "start_date";
      if (field_key === "duration")
        this.localFields[field_key].callback = this.durationUpdated;
      // Find longest table cell
      if (this.longest_cell.width < field_width)
        this.longest_cell = { width: field_width, slug: field_key };
    });
    // Check if all the required fields are provided
    // provided in the gantt props
    let valid_fields = this.requiredFields.every(field =>
  ...
  methods: {
    ...
    compileItems(items) {
      ...
      items.forEach(item => {
        ...
        earliest_date =
          new Date(item.start_date) < new Date(earliest_date)
            ? item.start_date
            : earliest_date;
        // If we have children
    ...
    /*
        ...
        */
    cellUpdated(callback, cell, field) {
      if (callback) callback(cell);
      this.$emit("update", cell, field);
    },
    /*
    ...
    startdateUpdated(cell) {
      ...
      if (cell.isChild && !cell.recursive) {
        ...
        let parent_item = this.localItems.find(
          item => item.id === parseInt(cell.parentId)
        );
        // Get all the parents children
        let children = this.localItems.filter(
          item => item.parentId === parseInt(cell.parentId)
        );
        ...
        children.forEach(child => {
          start_date =
            new Date(child.start_date) < new Date(start_date)
              ? child.start_date
              : start_date;
          end_date =
            new Date(child.end_date) > new Date(end_date)
              ? child.end_date
              : end_date;
        });
        let duration = this.$moment(end_date, this.dateFormat).diff(
    ...
    enddateUpdated(cell) {
      ...
      cell.end_date = this.$moment(cell.end_date, this.dateFormat)
        .endOf("day")
        .subtract(diff, "d")
        .startOf("day")
        .format(this.dateFormat);
*/
    ...
    durationUpdated(cell) {
      ...
      let end_date = this.$moment(cell.start_date, this.dateFormat).add(
        cell.duration - 1,
        "d"
      );
      ...
      if (cell.isChild) {
        ...
        let duration =
          this.$moment(end_date, this.dateFormat).diff(
            this.$moment(start_date, this.dateFormat),
            "d"
          ) + 1;
        if (!parent_item.ignoreCallback) parent_item.ignoreCallback = {};
    ...
    initDragable() {
      ...
      document.documentElement.onmouseup = e => {
        ...
        window.markers.forEach(marker => {
          ...
          let item = this.localItems.find(item => item.id === row_id);
        ...
        window.markers.forEach(marker => {
          ...
          for (let i = 1; i < cell_limit + 1; i++) {
            if (
              Math.floor(
                (i * this.widths.cell) /
                  (e.clientX + marker.offset - chart_offset)
              ) !== 0
            ) {
              match_cell = i;
              break;
    ...
    computedOffset(item) {
      let offset = this.$moment(this.localStartDate, this.dateFormat).diff(
        this.$moment(item.start_date, this.dateFormat),
        "d"
      );
